...
# Definir un umbral para las predicciones
threshold = 0.65

# Inicializar listas para errores con predicciones > umbral
male_misclassified_high_conf = []
female_misclassified_high_conf = []

# Procesar imágenes nuevamente para incluir el umbral
for category, info in folders.items():
    folder_path = info["path"]
    true_label = info["label"]
    ...
    for file_name in os.listdir(folder_path):
        ...
        for result in results:
            ...
            # Verificar si la predicción es incorrecta y tiene alta confianza
            if predicted_class != true_label and max(predictions[0]) > threshold:
                label = f"Pred: {class_labels[predicted_class]} ({max(predictions[0]):.2f}), Real: {class_labels[true_label]}"
                cv2.rectangle(image, (x, y), (x + width, y + height), (0, 0, 255), 2)  # Rectángulo rojo
                cv2.putText(image, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

                if true_label == 0:  # Male misclassified as Female
                    male_misclassified_high_conf.append(image)
                else:  # Female misclassified as Male
                    female_misclassified_high_conf.append(image)

# Mostrar imágenes mal clasificadas con alta confianza
def plot_high_conf_misclassified_images(images, title):
    plt.figure(figsize=(15, 10))
    n_cols = 5
    n_rows = (len(images) // n_cols) + (1 if len(images) % n_cols != 0 else 0)
...
# Mostrar imágenes mal clasificadas con alta confianza
plot_high_conf_misclassified_images(male_misclassified_high_conf, "Mal Clasificadas con Alta Confianza - Male Predichas como Female")
plot_high_conf_misclassified_images(female_misclassified_high_conf, "Mal Clasificadas con Alta Confianza - Female Predichas como Male")