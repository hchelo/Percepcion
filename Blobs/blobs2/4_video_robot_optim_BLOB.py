...
while cap.isOpened():
    ...
    im_with_contours = cv2.cvtColor(thresholded_image, cv2.COLOR_GRAY2BGR)
    blob_counter = 1
    ...
    for contour in contours:
        ...
        if M["m00"] != 0:
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            cv2.drawContours(im_with_contours, [contour], -1, (0, 0, 255), 2)
            cv2.line(im_with_contours, (cX - 10, cY), (cX + 10, cY), (0, 0, 255), 2)
            cv2.line(im_with_contours, (cX, cY - 10), (cX, cY + 10), (0, 0, 255), 2)
            cv2.putText(im_with_contours, str(blob_counter), (cX - 15, cY - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            blob_counter += 1

    # Mostrar el fotograma original y procesado
    cv2.imshow("Original", img)
    cv2.imshow("Procesado", im_with_contours)
