...
# Videos de salida (pon None en el que no quieras generar)
salida_flecha = 'salida_flecha.mp4'        # original + flecha
salida_filtrada = 'salida_filtrada.mp4'    # solo colores clasificados
...
ancho = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH) * escala)
alto = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT) * escala)
fourcc = cv2.VideoWriter_fourcc(*'mp4v')

writer_flecha = cv2.VideoWriter(salida_flecha, fourcc, fps, (ancho, alto)) if salida_flecha else None
writer_filtrada = cv2.VideoWriter(salida_filtrada, fourcc, fps, (ancho, alto)) if salida_filtrada else None

print(f"Procesando {total} cuadros ({ancho}x{alto} @ {fps:.1f} fps)...")
...
while cap.isOpened():
    ...
    # --- Guardar ---
    if writer_flecha:
        writer_flecha.write(con_flecha)
    if writer_filtrada:
        writer_filtrada.write(filtrada)

    n_frames += 1
    if n_frames % 30 == 0 or n_frames == total:
        pct = 100 * n_frames / total if total > 0 else 0
        vel = n_frames / (time.time() - t_inicio)
        print(f"  {n_frames}/{total} cuadros ({pct:.0f}%) - {vel:.1f} fps")

cap.release()
if writer_flecha:
    writer_flecha.release()
if writer_filtrada:
    writer_filtrada.release()

print(f"\nListo en {time.time() - t_inicio:.1f} s")
if salida_flecha:
    print(f"  -> {salida_flecha}")
if salida_filtrada:
    print(f"  -> {salida_filtrada}")